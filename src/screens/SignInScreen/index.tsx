...
const SignInScreen: React.FC<SignInProps> = ({navigation, route}) => {
  ...
  const handleSignIn = async () => {
    await auth()
      ...
      .then(() => navigation.reset({index: 0, routes: [{name: 'BottomTab'}]}))
      .catch(error => {
      ...
      <View style={{marginTop: SIZES.padding}}>
        <Shadow>
          <View style={styles.signInFormContainer}>
            <Text style={styles.title}>Đăng nhập để tiếp tục</Text>
              ...
              <FormInput
                ...
                appendComponent={
                  <IconButton
                    icon={
                      isVisiblePassword ? icons.eye_icon : icons.eye_off_icon
                    }
                    iconStyle={{
                      width: 20,
                      height: 20,
              ...
              <Text
                style={[
                  ...
                  {
                    color:
                      indicatorPassword === types.IndicatorPassword.INVALID_PASSWORD
                        ? COLORS.error
                        : COLORS.darkGreen,
                  },
                ]}>
            ...
            <View style={{marginTop: SIZES.padding}}>
              <CustomButton
                backgroundColor={(isEmailValid && isPasswordValid )? COLORS.primary : COLORS.gray}
